...
zelda2_location = {}
zelda2_location.__index = zelda2_location
...
function zelda2_location.new(name)
    local self = setmetatable({}, zelda2_location)
    if name then
...
function zelda2_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = zelda2_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function zelda2_location:connect_two_ways(exit, rule)
...
function zelda2_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function zelda2_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function zelda2_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function zelda2_location:accessibility()
...
-- 
function zelda2_location:discover(accessibility, keys)
...
entry_point = zelda2_location.new("entry_point")
